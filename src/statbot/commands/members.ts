...
import {addSeriesOptions, addMemberFilterOptions, handleStatbotRequest, addBotOption, addRequiredStatsOption,} from '../../utils/statBotApiUtils';

export class MembersCommand extends Subcommand {
    public constructor(context: Command.LoaderContext, options: Command.Options) {
        super(context, {
            ...options,
            name: "statbot-members",
            description: 'Query Statbot member data',
            detailedDescription: "Query Statbot API data for members",
    ...
    public override registerApplicationCommands(registry: Subcommand.Registry): void {
        registry.registerChatInputCommand((builder) =>
            builder
                .setName("statbot-members")
                .setDescription("Query Statbot member data")
                .addSubcommand(cmd => {
    ...
    public override async messageRun(message: Message): Promise<void> {
        ...
        if (args.length < 2) {
            await message.reply("Usage: `!statbot-members <sub> [key=value ...]`");
            return;
        }
