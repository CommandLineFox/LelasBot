...
export class ShortCommand extends Command {
    ...
    public constructor(context: Command.LoaderContext, options: Command.Options) {
        super(context, {
            ...options,
            name: 'short',
            description: 'Manage short alerts for the current YouTube channel',
            detailedDescription: 'Enable/disable short alerts, set the Discord channel, or set the mention role.',
    ...
    public override registerApplicationCommands(registry: Command.Registry): void {
        registry.registerChatInputCommand((builder) =>
            builder
                ...
                .addSubcommand(cmd =>
                    cmd.setName('enabled')
                        .setDescription('Enable or disable short alerts')
                ...
                .addSubcommand(cmd =>
                    cmd.setName('channel')
                        .setDescription('Set the Discord channel for short alerts')
                ...
                .addSubcommand(cmd =>
                    cmd.setName('role')
                        .setDescription('Set the mention role for short alerts')
                        .addRoleOption(opt =>
                            opt.setName('value')
    ...
    public override async chatInputRun(interaction: Command.ChatInputCommandInteraction): Promise<void> {
        ...
        switch (interaction.options.getSubcommand()) {
            case 'enabled': {
                ...
                result = await this.guildService.setShortEnabled(interaction.guildId!, currentId, value);
                break;
            }
            case 'channel': {
                const channel = interaction.options.getChannel('value', true) as TextChannel;
                result = await this.guildService.setShortDiscordChannelId(interaction.guildId!, currentId, channel.id);
                break;
            }
            case 'role': {
                const role = interaction.options.getRole('value', true) as Role;
                result = await this.guildService.setShortMentionRoleId(interaction.guildId!, currentId, role.id);
                break;
            }
        }
